...
def upgrade_billing(data_dir: Path) -> None:
	"""Migrate the billing database to the latest schema version.
	...
	"""
	data_dir.mkdir(parents=True, exist_ok=True)
	db_path = billing_db_path(data_dir)
	logger.info("migrating billing database %s", db_path)
	try:
...
def upgrade_all(data_dir: Path) -> None:
	"""Migrate the registry, the billing database and every existing user database.
	...
	if root.exists():
		for db_path in sorted(root.glob(f"*/{USER_DB_FILENAME}")):
			...
			migrated += 1
	logger.info("migrations done: registry + billing + %d user database(s)", migrated)
