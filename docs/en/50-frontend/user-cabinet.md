...
- **Applications (API)** — application management: creating an application, issuing and
  ...
  section is organized by applications. The menu item is a collapsible submenu: the first
  sub-item, "All applications", opens the overall list, the rest lead to the user's
  application pages. Inside the applications section the submenu starts expanded
  (otherwise the selected application is not visible); elsewhere it starts collapsed, and
...
- **Invoices** — the balance towards the gateway, the owner's fee invoices, the payment
  details and the choice of payment network
  ([ADR-0027](../20-architecture/decisions/0027-gateway-fee-billing.md)).
...
- **The application page**:
  ...
  - issuing an address from the panel — the "Issue an address" button in the users card
    ...
    registers the application user automatically.
...
The gateway owner takes a fee — a percentage of the user's turnover over a calendar
month, provided that turnover crossed a threshold
([ADR-0027](../20-architecture/decisions/0027-gateway-fee-billing.md)). Turnover means
confirmed incoming payments in the assets the operator designated; moving funds between
one's own addresses is not income. The section answers two questions: how much and where.

- **The balance** — a card on top, one figure: the sum of credits minus the sum of
  invoices. Zero or above — everything is in order; a negative one is highlighted, with
  the amount to pay next to it.
- **How to pay** (shown while in debt): the permanent payment address with a copy button
  and a note — an ordinary stablecoin transfer from any wallet or exchange. The gateway
  notices the money itself; an underpayment and an overpayment are not special cases but
  simply the size of the balance, and the remainder is topped up to the same address.
- **Invoice history**: period, turnover, rate, amount, due date, state (paid / awaiting
  payment / overdue). The state is computed from the balance: credits covering the
  invoice and everything older than it — paid; an uncovered invoice past its due date —
  overdue.
- **The payment network**: TRON by default; any network where the owner has set up a
  master wallet can be chosen instead. Changing it is refused while the balance is
  negative — the details of an issued invoice must not move.

### An overdue debt

A negative balance past its due date puts the cabinet into "payment only" mode: the
Application API refuses calls, and any cabinet section opens on the invoices page — the
debt and the payment details live there, otherwise the user would lose the means of
paying. Observation of their addresses does not stop: their payers keep paying and those
payments reach the history. Access returns automatically as soon as the balance stops
being negative; the operator can credit money by hand if it arrived outside the gateway.
