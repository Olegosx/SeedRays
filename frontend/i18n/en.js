// Interface dictionary: English.
export const en = {
	"login.pageTitle": "Sign in — SeedRays",
	"login.title": "Sign in to your cabinet",
	"login.identifier": "Username or email",
	"login.identifierPlaceholder": "username or you@example.com",
	"login.password": "Password",
	"login.passwordPlaceholder": "Your password",
	"login.forgot": "Forgot password?",
	"login.showPassword": "Show password",
	"login.remember": "Remember me on this device",
	"login.submit": "Sign in",
	"login.noAccount": "Don't have an account yet?",
	"login.register": "Sign up",
	"login.confirmedNotice": "Email confirmed — now sign in.",
	"login.confirmFailedNotice": "The confirmation link is invalid or expired.",
	"login.registeredNotice": "The account is created — sign in.",

	"errors.invalid_credentials": "Wrong username/email or password.",
	"errors.email_not_confirmed": "Confirm your email first — check your inbox.",
	"errors.username_taken": "This username is already taken.",
	"errors.email_taken": "This email is already attached to an account.",
	"errors.invalid_username": "Username: 3–64 characters, no spaces or '@'.",
	"errors.invalid_email": "This does not look like an email address.",
	"errors.weak_password": "The password is too short — at least 8 characters.",
	"errors.mail_failed": "Could not send the email — try again later.",
	"errors.mail_not_configured": "The gateway's outgoing mail is not configured yet — contact the operator.",
	"errors.rate_limited": "Too many attempts — wait a little and try again.",
	"errors.user_not_blocked": "Block the user first — only blocked users can be deleted.",
	"errors.username_mismatch": "The typed name does not match the user being deleted.",
	"errors.captcha_failed": "The robot-protection check failed — please try again.",
	"errors.network": "No connection to the server.",
	"errors.not_found": "No such address in the gateway API.",
	"errors.validation": "Check the fields you filled in.",
	"errors.method_not_allowed": "This address does not accept such a request.",
	"errors.request_failed": "The gateway refused the request.",
	"errors.internal_error": "The gateway failed to handle the request — look in the service log.",
	"errors.invalid_setting": "Invalid setting value — check the numeric fields.",
	"errors.invalid_xpub": "This xpub was not accepted — check that it is a valid account-level xpub.",
	"errors.private_key_rejected": "This is a PRIVATE key. Never paste it anywhere — treat it as compromised and move the funds to a new wallet. The gateway only needs the public xpub.",
	"errors.invalid_family": "Unknown wallet family.",
	"errors.invalid_words": "The phrase length is 12 or 24 words.",
	"errors.no_families": "Check at least one family.",
	"errors.invalid_name": "The name is 1 to 64 characters.",
	"errors.unknown_application": "No such application.",
	"errors.unknown_wallet": "No such wallet — refresh the wallet list.",
	"errors.unknown_network": "The gateway does not serve this network.",
	"errors.invalid_limit": "The page size is not a valid number.",
	"errors.invalid_state": "Unknown invoice state in the filter.",
	"errors.invalid_status": "Unknown status in the filter.",
	"errors.invoice_already_paid": "This invoice is already settled.",
	"errors.network_not_configured": "No wallet is set for this network yet.",
	"errors.reason_required": "State the reason first.",
	"errors.invalid_amount": "The amount must be a positive number of USDT.",
	"errors.debt_pending": "Settle the outstanding debt first: the payment details of an issued invoice must not change.",
	"errors.unknown_app_user": "No such user in this application.",
	"errors.unknown_invoice": "No such invoice.",
	"errors.unknown_setting": "Unknown setting.",
	"errors.unknown_user": "No such user.",
	"errors.wallet_family_mismatch": "This wallet belongs to another chain family — its addresses would not work in this network.",
	"errors.wallet_missing": "No such wallet.",
	"errors.unknown_email": "No such email on this account.",
	"errors.invalid_token": "The link is invalid or expired — request a new reset.",
	"errors.invalid_cursor": "Could not continue the list — reload the page.",
	"errors.cannot_remove_primary": "The primary email cannot be removed.",

	"register.pageTitle": "Sign up — SeedRays",
	"register.title": "Create an account",
	"register.email": "Email",
	"register.emailPlaceholder": "you@example.com",
	"register.username": "Username",
	"register.usernamePlaceholder": "username",
	"register.usernameHint": "A unique name; the @ character is not allowed.",
	"register.password": "Password",
	"register.passwordPlaceholder": "Choose a password",
	"register.submit": "Sign up",
	"register.haveAccount": "Already have an account?",
	"register.signIn": "Sign in",
	"register.sentTitle": "Confirm your email",
	"register.sentText": "We have sent a confirmation link to:",
	"register.sentHint": "Follow the link in the message to finish the registration.",
	"register.backToLogin": "Back to sign-in",

	"reset.pageTitle": "Password recovery — SeedRays",
	"reset.title": "Password recovery",
	"reset.text": "Enter your account email — we will send a password reset link.",
	"reset.email": "Email",
	"reset.emailPlaceholder": "you@example.com",
	"reset.submit": "Send the link",
	"reset.back": "Back to sign-in",
	"reset.sentTitle": "Email sent",
	"reset.sentText": "If this email is registered, a password reset link is on its way.",

	"newpass.pageTitle": "New password — SeedRays",
	"newpass.title": "New password",
	"newpass.text": "Choose a new password for signing in to the cabinet.",
	"newpass.password": "New password",
	"newpass.passwordPlaceholder": "Choose a password",
	"newpass.repeat": "Repeat the password",
	"newpass.repeatPlaceholder": "The same password again",
	"newpass.mismatch": "The passwords do not match.",
	"newpass.submit": "Save the password",
	"newpass.doneTitle": "Password changed",
	"newpass.doneText": "Now sign in with your new password.",
	"newpass.toLogin": "Sign in",

	"menu.dashboard": "Dashboard",
	"menu.wallets": "Wallets",
	"menu.apps": "Applications (API)",
	"menu.allApps": "All applications",
	"menu.history": "Operation history",
	"menu.settings": "Settings",
	"menu.logout": "Sign out",

	"dashboard.pageTitle": "Dashboard — SeedRays",
	"dashboard.title": "Dashboard",
	"dashboard.wallets": "Wallets",
	"dashboard.apps": "Applications",
	"dashboard.addresses": "Addresses",
	"dashboard.receiptsCard": "Receipts by asset",
	"dashboard.colReceived": "Total received",
	"dashboard.colPending": "Pending",
	"dashboard.recentCard": "Recent operations",
	"dashboard.allHistory": "Full history",

	"wallets.pageTitle": "Wallets — SeedRays",
	"wallets.title": "Wallets",
	"wallets.attach": "Attach a wallet",
	"wallets.generate": "Create in the gateway",
	"wallets.colLabel": "Label",
	"wallets.colFamily": "Family",
	"wallets.colAddresses": "Addresses",
	"wallets.colAdded": "Added",

	"attach.title": "Attach a wallet",
	"attach.hint":
		"The recommended path: the keys are created outside the gateway, and only the public key (xpub) is submitted. No secrets ever touch the gateway.",
	"attach.family": "Family",
	"attach.xpub": "Account-level xpub",
	"attach.xpubPlaceholder": "xpub6…",
	"attach.label": "Label",
	"attach.labelPlaceholder": "For example: Main",
	"attach.submit": "Attach",
	"attach.cancel": "Cancel",

	"gen.pageTitle": "Create a wallet — SeedRays",
	"gen.title": "Creating a wallet in the gateway",
	"gen.recommended":
		"The recommended way is to create the keys on your own machine (for example, with the standalone keygen utility) and give the gateway only the xpub — no secrets ever reach the server.",
	"gen.risk":
		"If exporting an xpub feels complicated, you can create a wallet right here. Keep in mind: the seed phrase will be shown only once and is stored nowhere — the service cannot remind you of it or recover it. Write the phrase down right away and keep it somewhere safe.",
	"gen.lengthLabel": "Seed phrase length",
	"gen.words12": "12 words",
	"gen.words24": "24 words",
	"gen.familiesLabel": "Families — one wallet per checked family",
	"gen.passphraseLabel": "Use a BIP39 passphrase",
	"gen.passphraseHint":
		"Optional: the passphrase is an extra password for the wallet. A mistyped password gives no error message — a non-existent wallet is simply opened.",
	"gen.passphraseWarning": "If you forget the passphrase, access to the funds is lost forever.",
	"gen.passphrasePlaceholder": "Passphrase",
	"gen.generate": "Generate",
	"gen.cancel": "Cancel",
	"gen.phraseTitle": "Write down the seed phrase",
	"gen.phraseWarning":
		"The phrase is shown exactly once and is stored nowhere. Anyone who knows it controls the funds. Write it down on paper and keep it offline.",
	"gen.written": "I wrote the phrase down — continue",
	"gen.confirmTitle": "Confirm the write-down",
	"gen.confirmText": "Enter the phrase words with the requested numbers:",
	"gen.word": "Word #",
	"gen.confirmError": "The words do not match the phrase.",
	"gen.confirm": "Confirm and create",
	"gen.doneTitle": "Wallets created",
	"gen.doneText":
		"The gateway stored only the xpubs — one per chosen family. The seed phrase is not stored.",
	"gen.toWallets": "To the wallet list",

	"apps.pageTitle": "Applications (API) — SeedRays",
	"apps.title": "Applications (API)",
	"apps.create": "Create an application",
	"apps.colName": "Name",
	"apps.colNetworks": "Networks",
	"apps.colUsers": "Users",
	"apps.colCreated": "Created",
	"apps.revoked": "key revoked",

	"appcreate.title": "New application",
	"appcreate.name": "Name",
	"appcreate.namePlaceholder": "For example: Online store",
	"appcreate.submit": "Create",
	"appcreate.cancel": "Cancel",

	"appkey.title": "Application key",
	"appkey.warning":
		"The key is shown only once: the gateway stores its fingerprint, not the key itself. Copy the key now and hand it to your application.",
	"op.loginPageTitle": "Operator sign-in — SeedRays",
	"op.loginTitle": "Operator panel",
	"op.login": "Login",
	"op.password": "Password",
	"op.submit": "Sign in",
	"op.menuUsers": "Users",
	"op.menuSettings": "Settings",
	"op.usersPageTitle": "Users — SeedRays",
	"op.usersTitle": "Gateway users",
	"op.colName": "Name",
	"op.colEmails": "Emails",
	"op.colStatus": "Status",
	"op.colWallets": "Wallets",
	"op.colCreated": "Created",
	"op.statusActive": "active",
	"op.statusBlocked": "blocked",
	"op.block": "Block",
	"op.unblock": "Unblock",
	"op.resetPassword": "Reset password",
	"op.confirmBlockTitle": "Block the user?",
	"op.confirmBlockText":
		"The user and all their applications lose access immediately; active sessions are terminated.",
	"op.confirmBlockGo": "Block",
	"op.confirmResetTitle": "Reset the user's password?",
	"op.confirmResetText":
		"The current password stops working and every session of the user is terminated. The new temporary password will be shown once.",
	"op.confirmResetGo": "Reset",
	"op.deleteUser": "Delete",
	"op.confirmDeleteTitle": "Delete this user?",
	"op.confirmDeleteText":
		"The account, wallets and history move into a server-side archive; the user disappears from the gateway and their name, emails and keys become free. Restoring is possible only with a server console command.",
	"op.confirmDeleteType": "Type the username to confirm",
	"op.confirmDeleteGo": "Delete",
	"op.tempPasswordTitle": "Temporary password",
	"op.tempPasswordText":
		"Shown once. Hand it to the user through a trusted channel and ask them to change it in the cabinet settings.",
	"op.settingsPageTitle": "Gateway settings — SeedRays",
	"op.settingsTitle": "Gateway settings",
	"op.secTron": "TRON provider (TronGrid)",
	"op.keyLabel": "API key",
	"op.keySet": "configured",
	"op.keyUnset": "not configured",
	"op.secretPlaceholder": "Leave empty to keep unchanged",
	"op.rateLabel": "Provider request rate, per second",
	"op.secMail": "Mail (Resend)",
	"op.mailFrom": "Sender address",
	"op.baseUrl": "Gateway base URL (for links in emails)",
	"op.devMode": "Development mode: auto-confirm emails, no messages are sent",
	"op.secWatcher": "Watcher",
	"op.intervalLabel": "Pass interval, seconds",
	"op.overlapLabel": "Scan overlap, minutes",
	"op.secNetwork": "Network & deployment",
	"op.trustedProxiesLabel": "Trusted reverse proxies (addresses and ranges, comma-separated)",
	"op.trustedProxiesHint": "Intermediaries (your nginx/Apache, Cloudflare ranges) whose X-Forwarded-For headers the gateway trusts when resolving the visitor IP. Empty — 127.0.0.1 only. \"*\" trusts everyone — dangerous unless the gateway is unreachable directly. Applied after a gateway restart.",
	"op.secSeclog": "Security journal",
	"op.seclogRotateLabel": "Journal file size before rotation, MB",
	"op.seclogBackupsLabel": "Compressed archives to keep",
	"op.seclogRestartHint": "Applied after a gateway restart. The journal lives at logs/security.log in the data directory.",
	"op.save": "Save",
	"op.saved": "Saved.",
	"op.numberNonNegative": "Enter a number of zero or more. An empty field means the default value.",
	"op.numberPercent": "A percentage with at most two decimal places",
	"op.numberPositiveInt": "Enter a whole number of one or more. An empty field means the default value.",
	"op.secWatcherStatus": "Watcher status",
	"op.colNetwork": "Network",
	"op.colLastBlock": "Last block",
	"op.colLastScan": "Token scan cursor",
	"op.never": "not yet",
	"menu.billing": "Invoices",
	"billing.pageTitle": "Invoices — SeedRays",
	"billing.title": "Gateway invoices",
	"billing.balance": "Balance",
	"billing.debt": "To pay",
	"billing.howToPay": "How to pay",
	"billing.howToPayHint": "Transfer a stablecoin to your permanent payment address — an ordinary transfer from any wallet or exchange. The gateway notices the money itself and credits it to your balance; access returns automatically once the balance is no longer negative.",
	"billing.payAddress": "Payment address",
	"billing.copy": "Copy",
	"billing.copied": "Copied",
	"billing.noAddress": "No payment address has been issued yet — contact the gateway operator.",
	"billing.invoices": "Invoice history",
	"billing.invoicesEmpty": "No invoices yet: the fee applies only when the monthly turnover crosses the threshold.",
	"billing.colPeriod": "Period",
	"billing.colTurnover": "Turnover, USDT",
	"billing.colRate": "Rate",
	"billing.colAmount": "Amount, USDT",
	"billing.colDue": "Due date",
	"billing.colState": "State",
	"billing.stateIssued": "Awaiting payment",
	"billing.stateOverdue": "Overdue",
	"billing.statePaid": "Paid",
	"billing.network": "Payment network",
	"billing.networkHint": "The network invoices are issued in and the payment address belongs to. While the balance is negative the network cannot change: the details of an issued invoice must not move.",
	"op.menuBilling": "Invoices",
	"op.billingPageTitle": "Invoices — SeedRays",
	"op.billingTitle": "The gateway fee",
	"op.masterWallets": "Master wallets",
	"op.masterWalletsHint": "An account-level watch-only xpub per payment network: every user gets a permanent invoice address derived from it. A network without a wallet is not offered to users, and no invoice is issued in it.",
	"op.colXpub": "Extended public key",
	"op.xpubPlaceholder": "account-level xpub",
	"op.chooseNetwork": "Choose a network",
	"op.add": "Add",
	"op.remove": "Remove",
	"op.invoices": "Users' invoices",
	"op.invoicesEmpty": "No invoices yet.",
	"op.colUser": "User",
	"op.colPeriod": "Period",
	"op.colTurnover": "Turnover, USDT",
	"op.colAmount": "Due, USDT",
	"op.colDue": "Due date",
	"op.colState": "State",
	"op.invoiceStateAll": "Any state",
	"op.invoiceStateIssued": "Awaiting payment",
	"op.invoiceStateOverdue": "Overdue",
	"op.invoiceStatePaid": "Paid",
	"op.confirmPayment": "Confirm payment",
	"op.confirmPaymentTitle": "Confirm a payment by hand",
	"op.confirmPaymentBody": "The invoice of {user} for {amount} USDT will be marked as paid and access to the gateway will return. This is how money that arrived outside the gateway is handled.",
	"op.colBalance": "Balance, USDT",
	"op.amountLabel": "Amount credited, USDT",
	"op.reasonLabel": "Reason",
	"op.reasonPlaceholder": "for example: paid by bank transfer",
	"op.cancel": "Cancel",
	"op.confirm": "Confirm",
	"op.secFee": "The gateway fee",
	"op.feeEnabled": "Charge users the gateway fee",
	"op.feeRateLabel": "Rate, % of turnover",
	"op.feeRateHint": "Empty means no fee is charged at all — the placeholder is only an example.",
	"op.feeThresholdHint": "Empty means every turnover is billed, however small.",
	"op.feeThresholdLabel": "Monthly turnover threshold, USDT",
	"op.feeDueDaysLabel": "Payment term, days",
	"op.feeRemindLabel": "Remind before the due date, days (empty — never)",
	"op.feeOwnerEmailLabel": "Owner's email for service mail",
	"op.feeTurnoverAssets": "Assets counted towards turnover:",
	"op.feePaymentAssets": "Assets accepted as payment:",
	"op.feeHint": "Assets are given as a JSON list of contract addresses. Symbols are not trusted: the catalog fills with whatever arrives on an address, and a counterfeit \"USDT\" would otherwise inflate the turnover. An empty list means nothing is counted or accepted in that network.",
	"op.contractListHint": "Give a JSON list of contract addresses, for example [\"TR7NHqjeKQxGTCi8q8ZY4pL8otSzgjLj6t\"]. An empty field means nothing is counted.",
	"op.secPassword": "Operator password change",

	"confirm.cancel": "Cancel",
	"confirm.reissueTitle": "Reissue the key?",
	"confirm.reissueText":
		"The old key stops working immediately — the application loses access until the new key is put into it. The new key will be shown once.",
	"confirm.reissueGo": "Reissue",
	"confirm.revokeTitle": "Revoke the key?",
	"confirm.revokeText":
		"The application loses API access immediately. Access can only be restored by reissuing the key.",
	"confirm.revokeGo": "Revoke",

	"appkey.copy": "Copy",
	"appkey.copyFailed": "Could not copy automatically — select the key and copy it manually.",
	"appkey.copied": "Copied",
	"appkey.done": "Done",

	"app.pageTitle": "Application — SeedRays",
	"app.back": "To the application list",
	"app.keyCard": "API key",
	"app.keyIssued": "Issued",
	"app.keyPrefix": "The key starts with",
	"app.keyHint":
		"The key itself is not stored and cannot be shown again — only its first characters are kept in the open, to identify the key. If the key is lost, reissue it.",
	"app.reissue": "Reissue the key",
	"app.revoke": "Revoke the key",
	"app.revoked": "The key is revoked — the application has no API access.",
	"app.networksCard": "Networks and wallets",
	"app.networksHint":
		"The application works only with the networks in this list; addresses are issued from the specified wallet.",
	"app.colNetwork": "Network",
	"app.colWallet": "Wallet",
	"app.addNetwork": "Add a network",
	"app.remove": "Remove",
	"app.usersCard": "Application users",
	"app.colInstance": "Instance",
	"app.instanceDefault": "default",
	"app.colUser": "Identifier",
	"app.colAddresses": "Addresses",
	"app.colUserCreated": "Created",
	"app.addressesEmpty": "No addresses yet.",
	"app.issue": "Issue an address",
	"app.issueTitle": "Address issuance",
	"app.issueHint":
		"Give the application user's identifier — the same one the application uses for them in the API. An unknown identifier is registered automatically; issuing again returns the addresses already given out.",
	"app.issueInstance": "Application instance",
	"app.issueInstancePlaceholder": "blank — the default instance",
	"app.issueInstanceHint":
		"Fill this in when several independent installations of the application share one key: the instance keeps their user identifiers apart. An application with a single installation leaves the field empty.",
	"app.issueUser": "User identifier",
	"app.issueUserPlaceholder": "for example, user1",
	"app.issueNetworks": "Networks",
	"app.issueNoNetworks":
		"Add at least one network in the \"Networks and wallets\" card first — addresses are issued from it.",
	"app.issueGo": "Issue",
	"app.issueCancel": "Cancel",
	"app.issueClose": "Done",
	"app.issueResult": "The user's addresses",

	"history.pageTitle": "Operation history — SeedRays",
	"history.title": "Operation history",
	"history.filterAll": "All",
	"history.colTime": "Time",
	"history.colWallet": "Wallet",
	"history.colNetwork": "Network",
	"history.colAsset": "Asset",
	"history.colAmount": "Amount",
	"history.colTxid": "Transaction",
	"history.colStatus": "Status",
	"history.statusConfirmed": "confirmed",
	"history.statusPending": "pending",
	"history.statusFailed": "failed",
	"history.showMore": "Show more",
	"history.empty": "No operations match the chosen filters.",

	"settings.pageTitle": "Settings — SeedRays",
	"settings.title": "Settings",
	"settings.profileCard": "Profile",
	"settings.username": "Username",
	"settings.email": "Email",
	"settings.emailPrimary": "primary",
	"settings.emailConfirmed": "confirmed",
	"settings.emailUnconfirmed": "not confirmed",
	"settings.emailHint": "An added email must be confirmed by a message.",
	"settings.addEmail": "Add an email",
	"settings.add": "Add",
	"settings.emailPlaceholder": "you@example.com",
	"settings.passwordCard": "Password change",
	"settings.currentPassword": "Current password",
	"settings.newPassword": "New password",
	"settings.repeatPassword": "Repeat the password",
	"settings.changePassword": "Change the password",
	"settings.passwordChanged": "The password is changed.",
	"settings.methodsCard": "Sign-in methods",
	"settings.methodPassword": "Password",
	"settings.methodSet": "set",
	"settings.emailSent": "A message with the confirmation link has been sent.",
	"settings.methodsSoon": "Signing in through external services arrives in later stages.",
};
